...
@st.cache_data
def load_data():
    url = "https://docs.google.com/spreadsheets/d/1Fe3-vtF8WRj0XvP6lhGvgsjM_PDWd6g8ZKyR6_0cf4Q/gviz/tq?tqx=out:csv&sheet=Hoja1"
    df = pd.read_csv(url)
    return df
...
if st.button("Descubrir mi café ideal"):
    ...
    if recomendados.empty:
        ...
        for _, row in recomendados.iterrows():
            ...
            st.markdown(f"- **Intensidad:** {row['Intensidad']} | **Ética:** {row['Práctica Ética']}")
            if row["Link Shopify"]:
